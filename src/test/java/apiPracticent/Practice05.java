...
public class Practice05 extends TestBase{
	...
	*/
	...
	public void Practice() {
		...
		putReqBody.put("employee_name","Ali Can");
		...
		response.
		      then().
		      assertThat().
		      statusCode(200).body("status", equalTo(expectedResBody.getString("status")),
		    		  "data.empty", equalTo(expectedResBody.getBoolean("data.empty")),
		    		  "message", equalTo(expectedResBody.getString("message")));
		
		/*
		  When you send "patch" request, you are sending just a couple of the data not all.
		  But the response body contains all data. If the test case asks you to assert all
		  data request body object will not be enought to assert all details.
		  
		  So you need to create a new object and store expected data on it to use in assertion.
		 */
